...
function preload() {
	img = loadImage('../img/mountain.jpg')
}

function setup() {
	createCanvas(screen.width-20, img.height)
	background(0)
	
	for(var i = 0; i < total; i++){
		circles[i] = {};
		circles[i].prevPos = {x: screen.width/2, y: height/2}
		circles[i].pos = {x: screen.width/2, y: height/2}
